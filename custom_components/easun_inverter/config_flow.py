...
class EasunInverterConfigFlow(
    ...
    async def async_step_user(self, user_input=None):
        """Initial step: ask for inverter_ip, local_ip, model & scan_interval."""
...
class EasunInverterOptionsFlow(config_entries.OptionsFlow):
    ...
    def __init__(self, config_entry):
        ...
        _LOGGER.debug("OptionsFlow init for entry %s", config_entry.entry_id)
    ...
    async def async_step_init(self, user_input=None):
        ...
        if user_input is not None:
            ...
            # Returning 'data' here makes user_input become config_entry.options
            return self.async_create_entry(title="", data=user_input)

        # Pre‐populate the form from existing data + options
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    "inverter_ip",
                    default=opts.get("inverter_ip", data["inverter_ip"]),
                ): str,
                vol.Required(
                    "local_ip",
                    default=opts.get("local_ip", data["local_ip"]),
                ): str,
                vol.Required(
                    "model",
                    default=opts.get("model", data["model"]),
                ): vol.In(MODEL_KEYS),
                vol.Required(
                    "scan_interval",
                    default=opts.get(
                        "scan_interval",
                        data.get("scan_interval", DEFAULT_SCAN_INTERVAL),
                    ),
                ): vol.All(vol.Coerce(int), vol.Range(min=1, max=3600)),
            }
        )
