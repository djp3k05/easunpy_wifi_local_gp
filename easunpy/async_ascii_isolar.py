...
class AsyncAsciiISolar:
    ...
    def _parse_qpiri(self, raw: str) -> dict:
        f = raw.strip("()").split()
        # (same mapping from your PS1 script, friendly names)
        ...
        omode  = {
            ...
            "02": "Phase 1 of 3 Phase",  "03": "Phase 2 of 3 Phase",
            "04": "Phase 3 of 3 Phase",  "05": "Phase 1 of 2 Phase",
            "06": "Phase 2 of 2 Phase",  "07": "Phase 2 of 2 Phase (180°)",
        }.get(f[21], f[21])
    ...
    def _parse_qpiws(self, raw: str) -> list[str]:
        ...
        return [name for idx, name in warn_map.items() if idx < len(bits) and bits[idx] == '1'] or ["No warnings"]

    async def get_all_data(self) -> tuple[BatteryData, PVData, GridData, OutputData, SystemStatus]:
        raws: dict[str, str] = {}
        for cmd in self.commands:
            raws[cmd] = await self._send_command(cmd)
            await asyncio.sleep(0.3)

        # close & reset so next poll re-discovers
        if self._writer:
            self._writer.close()
        ...
        p1    = self._parse_qpigs(raws["QPIGS"])
        p2    = self._parse_qpigs2(raws["QPIGS2"])
        mode  = self._parse_qmod(raws["QMOD"])
        info  = self._parse_qpiri(raws["QPIRI"])
        warns = self._parse_qpiws(raws["QPIWS"])
